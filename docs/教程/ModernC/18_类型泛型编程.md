...
类型泛型特性已深深融入 C，以至于大多数程序员或许根本没有意识到它们无处不在。我们首先辨认出贯穿本书、已经见过的若干类型泛型特性，并更系统地审视它们（见 18.1 节）。讨论涵盖适用于多种类型的运算符、类型提升与转换、宏、可变参数函数、函数指针和 `void` 指针。随后介绍 C11 带来的一项更复杂、可编程的特性：借助关键字 `_Generic` 进行泛型选择（18.2 节）。它让我们能够在编译时依固定的一组类型区分不同动作——例如面对 `float`、`double` 或 `long double` 值时，选择函数的不同变体。

`_Generic` 的一项缺点是，它通常只能处理预先规定的有限类型集合，因而很快就会导致情况组合数量爆炸。C23 引入一项称为**类型推断**的新特性（关键字 `auto`、`typeof` 和 `typeof_unqual`），它可以从给定表达式中一致地推断类型信息，避免编写这种复杂代码（18.3 节）。

遗憾的是，C23 还没有提供 C 类型泛型编程中经常使用的另一项特性：函数式抽象。主流编译器提供了几种实现此类特性的扩展。由于它们对代码的简洁性、泛型能力、安全性和性能都很重要，18.4 节将简要回顾其中一些扩展。
...
要实现真正的泛型，关键在于不能对可能传入的类型做任何预先假设。在声明中使用 `auto` 关键字而不是具体的类型名，可以确保声明出来的变量类型与每次实际调用时传入的参数类型完全一致。因为 `swap_p1` 和 `swap_p2` 都是通过带有 `&` 一元运算符的表达式初始化的，所以它们的类型分别是“指向表达式 `X` 所属类型的指针”和“指向表达式 `Y` 所属类型的指针”。`swap_tmp` 的类型则是 `swap_p1` 所指向的类型，即表达式 `X` 的原始类型。除了使用 `auto` 之外，这两个指针还被 `const` 关键字修饰，这保证了在这段代码内部，指针本身的指向是不允许被改变的。
