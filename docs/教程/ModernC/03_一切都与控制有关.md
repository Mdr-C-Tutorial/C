...
下面的示例展示了 `while` 的一种典型用法。它实现 Heron 近似，用于计算数 `a` 的乘法逆元 $\frac{1}{a}$：

（译者注：这个程序有 bug。下方讲解 Heron 近似的折叠块内会说明 bug 出现的原因，更下方的**清单 3.1** 给出了更完整、无 bug 的程序）
...
这些示例使用了 `<tgmath.h>` 头文件中的标准宏 `fabs`。[^4] 它用于计算 `double` 值的绝对值。清单 3.1 展示了实现该算法的完整程序，不同之处在于它没有使用 `fabs`，而是显式地与几个固定数值进行比较。例如，`eps1m24` 被定义为 $1-1\cdot2^{-24}$，`eps1p24` 被定义为 $1+1\cdot2^{-24}$。在后面的内容中（第 5.3 节），你会了解到这些定义里使用的字面量 `0x1P-24`（代表数值 $2^{-24}$）及相关语法是如何工作的，以及它们怎样与 `constexpr` 构造（第 5.6.5 节）结合使用。
