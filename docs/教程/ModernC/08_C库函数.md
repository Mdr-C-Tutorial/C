...
C 标准提供的功能分为两大部分：一部分是 C 语言本身，另一部分是 C 库。我们已经看过几个 C 库函数，包括 `printf`、`puts` 和 `strtod`，所以你应当大致知道会遇到什么：日常编程所需的基本工具，以及为确保可移植性而必须具备明确接口和语义的功能。
...
**平台抽象层。** 这类函数把平台的特有性质和需求抽象掉。实现输入／输出等基本操作需要平台特有的部分；若不了解平台的深层细节，就无法自行实现。例如，`puts` 必须知道何谓“终端输出”，以及如何访问终端。实现这些功能需要操作系统甚至处理器特有的技巧，超出了大多数 C 程序员的知识范围。庆幸有人已经替你完成了这项工作吧。
...
`printf` 这样的函数可以看作兼有两种用途：它实际上可以分成格式化阶段和输出阶段，前者提供基本工具，后者依赖具体平台。函数 `snprintf`（很久以后才会在 14.1 节说明）提供与 `printf` 相同的格式化功能，但把结果存入字符串。之后可用 `puts` 打印这个字符串，从而得到与整个 `printf` 相同的输出。
...
在当前层次，我们将讨论使用此前介绍的语言元素进行基本编程时不可或缺的 C 库函数。进入更高层次、讨论更多概念时，还会继续补全这些内容。表 8.1 概览了各个标准头文件。
...
与此前一样，这只进行文本替换；由于替换文本可能多次包含宏实参，因此绝不能向这样的宏或函数传入带有副作用的表达式。希望此前关于副作用的讨论（要点 4.3 #2）已经让你确信不该这样做。

接下来要看的某些接口，其实参或返回值是指针。我们还无法完整处理这些情况，但在多数场合，指针实参只需传入已知指针或 `nullptr` 即可。指针作为返回值，只会出现在可将其解释为错误状况的情形中。
...
如果调用 C 库中的许多函数时传入的一组形参不一致，它们就容易发生**缓冲区溢出**。这已经导致（并仍在导致）大量安全缺陷和漏洞利用，通常必须极其谨慎地处理。
...
使用 C 这样的标准化语言编程，一项重要目标就是可移植性。我们应尽量少对执行平台作出假设，把空缺留给 C 编译器和库来补足。遗憾的是，有时无法做到这一点；此时就应清楚标明代码的前置条件。
...
可以看到，这种条件指令以某行中的记号序列 `#if` 开始，以另一行中的 `#endif` 结束。只有条件（这里是 `!__STDC_LIB_EXT1__`）为真时，才执行中间的 `#error` 指令。它以一条错误消息中止编译过程；类似的 `#warning` 指令允许编译继续，但保证给出警告消息。可以放入这种构造的条件受到限制。[练习 62]
...
有几个函数提供常见整数算术。前两个函数族 `abs` 和 `div` 来自头文件 `<stdlib.h>`；前缀 `l` 表示 `long` 实参，`ll` 表示 `long long` 实参。`abs` 函数是为方便使用而提供的接口，因为 C 没有表示有符号整数绝对值的紧凑记法。即便写成表达式：
...
这些函数的返回类型是结构体，不过不必太在意这些结构体的名称。C23 允许像上面那样用 `auto` 推断类型，从而更容易接住返回值。对三个 `div` 函数，都可以通过成员 `quot` 和 `rem` 访问具体结果。如今，优化编译器通常很擅长把商和余数运算合并成一条指令，因此这些接口已经不再十分有用。
...
其余函数涉及位操作。它们同样由 C23 通过新头文件 `<stdbit.h>` 引入，接收任意标准或扩展无符号整数类型的值作为实参。其返回值如表 8.5 所示：第二列给出一般公式，第三列给出一般公式不适用的例外情形所对应的结果。请注意：

- 所有函数对所有实参值都有定义好的结果。即使编译器可以对大多数值使用某条硬件指令来实现函数，也应由编译器而不是你来正确处理特殊情形。
...
- 以下函数的结果与实参类型无关：
  ...
  尤其是最后两个函数，与具有类似功能的其他变体相比，能让你和代码的其他读者轻松不少。对这六个函数，只要可以，几乎总应优先使用类型泛化版本的接口。
...
如今，数值函数的实现理应质量优良、效率很高，并且对数值精度有严格控制。具备足够数值知识的程序员固然可以实现其中任何函数，但你不应试图替换或绕开它们。其中许多不只是 C 函数，还可以使用处理器特有指令。例如，处理器可能提供 `sqrt` 和 `sin` 的快速近似，或者以底层指令实现浮点乘加 `fma`。尤其是，检查或修改浮点内部表示的所有函数都很可能使用这类底层指令，例如 `carg`、`creal`、`fabs`、`frexp`、`ldexp`、`llround`、`lround`、`nearbyint`、`rint`、`round`、`scalbn` 和 `trunc`。因此，替换它们，或用手写代码重新实现它们，通常都不是好主意。
...
如果要把输出写入真正的文件，就必须借助 `fopen` 函数把文件附接到程序执行中：
...
打开文件可能因为多种原因失败，例如文件系统已满，或者进程没有在指定位置写入的权限。我们检查这种错误状况（要点 8.1.3 #2），并在必要时退出程序。
...
打开文件的模式不止一种，`"a"` 只是若干可能之一。表 8.7 概览了模式字符串中可以出现的字符。三个基本模式控制已有文件会发生什么，以及流位于何处；此外，还可以在其后附加三个修饰符。表 8.8 完整列出了各种可能组合。
...
`freopen` 和 `fclose` 的主要用途直截了当。`freopen` 可以把给定流关联到另一个文件，并最终改变模式。它对于把标准流关联到文件尤其有用。例如，刚才的小程序可以改写为：
...
宽度（`WW`）和精度（`.PP`）可以控制打印值的总体外观。例如，对通用浮点格式 `"%g"`，精度控制有效数字的位数。格式 `"%20.10g"` 规定一个 20 字符宽、至多包含 10 位有效数字的输出字段。不同格式说明符对这些值的具体解释并不相同。

标志可以改变输出形式，例如为结果添加符号（`"%+d"`），为十六进制转换添加 `0x`（`"%#X"`），为八进制添加 `0`（`"%#o"`）；以 `0` 填充；或者让输出在字段中左对齐而不是右对齐。见表 8.13。请记住，整数前导零通常被解释为引出八进制数，而不是十进制数。因此，把零填充与左对齐组合成 `"%-0"` 并不好，因为这会让读者混淆所采用的约定。
...
历史上，本着 `puts` 特化 `fputs` 的同一思路，先前版本的 C 标准曾提供 `gets` 接口。它处理缓冲区溢出的方式内在地不安全，因此已经从 C 标准中删除。
...
这是 `cat` 的一个小型实现：它读取命令行给出的若干文件，并把内容转储至 `stdout`。[练习 64][练习 65][练习 66][练习 67]
...
C 中的字符串处理必须面对这样一个事实：源环境与执行环境可以使用不同编码。因此，提供不依赖具体编码的接口至关重要。最重要的工具由语言本身提供：`'a'` 和 `'\n'` 这样的整数字符常量，以及 `"hello:\tx"` 这样的字符串字面量，在你的平台上应当始终产生正确效果。你也许还记得，宽度小于 `int` 的类型不一定有相应字面量。作为历史遗留，`'a'` 这样的整数字符字面量具有 `int` 类型，而不是你大概会以为的 `char`。[^68] 处理字符类别时，这类字面量可能会变得很麻烦。
...
第二个形参可用于取得剩余数据的位置，不过这对我们来说仍然太复杂。目前只需给该形参传入 `0`，就能保证一切正常。常用而方便的一组实参是 `strtoul(S, 0, 0)`，它会尝试把 `S` 解释为数，而不考虑输入格式。三个返回浮点值的函数工作方式类似，只是函数形参数量限为两个。
...
为了跳过空白，`Strtoul` 使用 `<string.h>` 提供的字符串搜索函数之一 `strspn`。该函数返回第一个形参开头的一段序列的长度；这段序列完全由第二个形参中的任意字符组成。函数 `strcspn`（`c` 代表 complement，即补集）的工作方式类似，不过它寻找的是一段不含第二个实参中任何字符的起始序列。
...
这个函数之所以弃用，是因为它使用静态缓冲区返回结果。因此，后续调用或并行调用都会抹掉字符串中已经存储的内容。
...
如果需要更高精度，可以使用 `struct timespec` 和 `timespec_get` 函数。这样就多了成员 `tv_nsec`，提供纳秒精度。第二个实参 `base` 只有一个 C 标准强制要求的值：`TIME_UTC`。以该值调用 `timespec_get`，结果应与 `time` 调用一致；从 C23 起，还可用 `timespec_getres` 查询该时钟的分辨率。它们都以地球的参考时间为准。其他时钟可能相对于你的计算机系统所处的行星系统或其他物理系统。[^74] 尤其是，C 标准库还提供另一个接口，用于收集归属于当前执行的处理时间：
...
但在调试阶段，它很有帮助，可以把你带到那些对各对象取值作出错误假设的位置。
