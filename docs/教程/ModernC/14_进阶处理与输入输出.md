...
既然已经了解指针及其工作方式，现在可以从新的角度审视 C 库的一些功能。C 的文本处理离不开指针，因此本章先在第 14.1 节考察一个更详尽的示例。随后会介绍格式化输入函数（第 14.2 节）；这些函数要求以指针作为实参，所以必然要把介绍推迟到这里。接着会出现一整套处理扩展字符集（第 14.3 节）和二进制流（第 14.6 节）的新函数。本章以及整个层级的最后，则会讨论如何干净利落地处理错误（第 14.6 节）。
...
如前所述，只要环境保持一致，也就是源码内部、其他文本文件和终端都使用相同的多字节编码，那么多字节字符串的全部编码工作与简单输入输出就能顺利进行。遗憾的是，并非所有环境都已经使用同一种编码，因此在不同环境之间传输文本文件（包括源文件）或可执行文件时，可能遇到困难。除定义庞大的字符表外，Unicode 还定义了三种如今广泛使用、并有望最终取代所有其他编码的编码：UTF-8、UTF-16 和 UTF-32，分别表示采用 8 位、16 位和 32 位字的 Unicode 转换格式。
