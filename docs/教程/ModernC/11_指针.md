...
指针是深入理解 C 时遇到的第一道真正难关。当我们需要从代码的不同位置访问对象，或者需要在运行过程中动态组织数据时，就会用到指针。

初学者把指针和数组混为一谈的现象可谓声名狼藉，所以请注意：要准确掌握相关术语，可能会遇到一些困难。另一方面，指针又是 C 最重要的特性之一。它们极大地帮助我们摆脱特定平台的琐碎细节，建立抽象并编写可移植代码。因此，学习本章时请保持耐心，因为它对于理解本书之后的大部分内容都至关重要。
...
此前已经见过不同类型的若干**表示**，也就是平台把特定类型的值存入对象的方式。一种类型（例如 `size_t`）的表示，对另一种类型（例如 `double`）可能毫无意义。只要直接使用对象，C 的类型系统就会保护我们免遭这些表示的混淆：`size_t` 对象始终按 `size_t` 访问，绝不会被解释为毫无意义的 `double`。
