...
page_t *get_page(uint32_t address, int make, page_directory_t *dir, bool ist) {
    ...
    if (dir->tables[table_idx]) {
        ...
        dir->tables[table_idx] =
            (page_table_t *)kmalloc_i_ap(sizeof(page_table_t), &tmp);
...
static page_table_t *clone_table(page_table_t *src, uint32_t *physAddr) {
    page_table_t *table =
        (page_table_t *)kmalloc_i_ap(sizeof(page_table_t), physAddr);
...
page_directory_t *clone_directory(page_directory_t *src) {
    ...
    page_directory_t *dir =
        (page_directory_t *)kmalloc_i_ap(sizeof(page_directory_t), &phys);
...
void init_page(multiboot_t *mboot) {
    ...
    kernel_directory = (page_directory_t *)kmalloc_ap(
        sizeof(page_directory_t),
        &physical_addr); // kmalloc: 无分页情况自动在内核后方分配 |
                         // 有分页从内核堆分配
    ...
    unsigned int j = mboot->framebuffer_addr, size = mboot->framebuffer_height *
    ...
    printf("GraphicsBuffer: 0x%08x - 0x%08x \n", (mboot->framebuffer_addr),
           (mboot->framebuffer_addr + size));
    printf("KernelHeap: 0x%08x - 0x%08x | ", (KHEAP_START),
