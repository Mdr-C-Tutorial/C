...
static json_value *parse_string(json_parser *p) {
    ...
    while (p->cur < p->end) {
        ...
        char esc = *p->cur++;
        switch (esc) {
            case '"':  if (sb_push_char(&sb, '"')  != 0) goto oom; break;
            case '\\': if (sb_push_char(&sb, '\\') != 0) goto oom; break;
            case '/':  if (sb_push_char(&sb, '/')  != 0) goto oom; break;
            case 'b':  if (sb_push_char(&sb, '\b') != 0) goto oom; break;
            case 'f':  if (sb_push_char(&sb, '\f') != 0) goto oom; break;
            case 'n':  if (sb_push_char(&sb, '\n') != 0) goto oom; break;
            case 'r':  if (sb_push_char(&sb, '\r') != 0) goto oom; break;
            case 't':  if (sb_push_char(&sb, '\t') != 0) goto oom; break;
            case 'u': {
                uint32_t cp1 = 0;
