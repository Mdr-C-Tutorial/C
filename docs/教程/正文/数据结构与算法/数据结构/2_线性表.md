...
1. 始终保持 `size <= capacity`，并保证 `capacity == 0` 当且仅当 `items == NULL`。
2. 插入位置可以等于 `size`。`count` 为 0 时插入成功，此时 `source` 可以为 `NULL`。
3. 当 `count > 0` 时，`source` 必须有效，且不得与顺序表拥有的存储区域重叠。
4. 插入时使用几何增长扩容；参数无效、大小计算溢出或分配失败时返回 `false`，并保持原顺序表不变。
5. 删除区间必须完全位于已有元素范围内。删除成功后保持剩余元素的相对顺序，不要求缩容。
6. 区间搬移必须使用适合重叠存储区域的标准库函数。

</Exercise>

<Exercise id="30003" :d="4" :w="4">
...
1. `int_chain_create` 返回的链表由调用方拥有；链表拥有其全部结点。
2. 头部插入、尾部插入和头部删除的时间复杂度均为 $O(1)$，删除首个指定值的结点为 $O(n)$。
3. 分配失败或大小无法表示时，插入函数返回 `false`，链表保持不变。
4. 空链表或参数无效时，`int_chain_pop_front` 返回 `false`，不得修改输出对象。
5. 删除最后一个结点后，头指针和尾指针都必须恢复为空指针。
6. `int_chain_destroy(NULL)` 应当安全，并且不得泄漏结点。

</Exercise>

<Exercise id="30015" :d="4" :w="4">
...
1. 使用环形存储，同时支持两端插入和删除；从空队列弹出或参数无效时返回 `false`，不得修改输出对象。
2. 扩容时保持从首端到尾端的逻辑顺序；分配失败或大小计算溢出时操作失败，队列保持不变。
3. `int_deque_destroy(NULL)` 安全，`int_deque_size(NULL)` 返回 0。
4. 两端操作的摊还时间复杂度为 $O(1)$；说明首下标、元素数量和容量之间的不变式。
