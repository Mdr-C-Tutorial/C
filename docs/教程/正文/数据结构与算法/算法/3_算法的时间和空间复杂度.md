...
4. **函数的增长不一定能比较**：
   ...
   - 当 $\sin n \to 1$ 时（例如 $n \approx 2k\pi + \frac{\pi}{2}$），$g(n) \approx n^2$。在这种情况下，$g(n)$ 的增长速度远快于 $f(n)$
...
<Exercise id="30301" :d="1" :w="2">

_[3.3M]_ 根据增长速度排序以下函数。
