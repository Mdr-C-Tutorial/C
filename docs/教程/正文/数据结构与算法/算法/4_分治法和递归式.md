...
1. 对 b 的幂证明
   ...
   4. **第一部分（递归终止项）：** $a^{\log_b n} T\left(\frac{n}{b^{\log_b n}}\right)$
      ...
      我们知道 $b^{\log_b n} = n$ (对数的定义)。
      所以，$\frac{n}{b^{\log_b n}} = \frac{n}{n} = 1$。
...
假设我们要计算两个 $n$ 位（或 $n$ 个字长）大整数 $X$ 和 $Y$ 的乘积。为了方便分析，我们设 $n$ 是 2 的幂。
...
因此，递归式为：
$T(n) = 4T(n/2) + O(n)$
根据主定理，其中 $a=4, b=2, f(n) = O(n)$。
$\log_b a = \log_2 4 = 2$。
由于 $f(n) = O(n) = O(n^{2-\varepsilon})$ 其中 $\varepsilon=1 > 0$，属于主定理情况 1。
所以，$T(n) = \Theta(n^2)$。这就是传统算法的时间复杂度。
...
1.  **得到递归式：**
    ...
    $$
2.  **使用主定理：**
    其中 $a=3, b=2, f(n) = \Theta(n)$。
    计算 $\log_b a = \log_2 3 \approx 1.585$。
    我们比较 $f(n)$ 与 $n^{\log_b a} = n^{\log_2 3}$。
    $f(n) = \Theta(n) = O(n^{\log_2 3 - \varepsilon})$，其中 $\varepsilon = \log_2 3 - 1 \approx 0.585 > 0$。
    这符合主定理的**情况 1**。
    因此，$T(n) = \Theta(n^{\log_b a}) = \Theta(n^{\log_2 3})$。
