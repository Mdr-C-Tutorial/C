...
1. 返回把 `left` 转换成 `right` 所需的最少操作次数。
2. 长度为 0 时，对应序列指针可以为 `NULL`。
3. 只保存动态规划表的一行，额外空间复杂度为 $O(\min(n,m))$。
4. 检查行存储的大小计算和分配结果。
5. 参数无效或无法完成计算时返回 `false`，并保持输出对象不变。
6. 时间复杂度为 $O(nm)$。

</Exercise>

<Exercise id="30804" :d="4" :w="3">
...
1. 重量为 0 的物品也只能选择一次；输入数组不得修改。
2. 参数无效、容量数组分配失败或价值加法溢出时返回 `false`，并保持输出对象不变。
3. 使用一维状态数组，并从大到小更新容量，时间复杂度为 $O(nC)$，额外空间复杂度为 $O(C)$。
4. 明确 `dp[c]` 在每次更新前后的语义，并用最小样例验证不能重复选择同一件物品。

</Exercise>

<Exercise id="30805" :d="3" :w="3">
...
1. 子序列保持原顺序，且相邻选中元素必须严格递增；相等元素不能同时计入同一条子序列。
2. `count == 0` 时返回长度 0，此时 `items` 可以为 `NULL`。
3. 参数无效或辅助数组分配失败时返回 `false`，不得修改输出对象。
4. 先使用“以第 `i` 个元素结尾”的状态写出 $O(n^2)$ 动态规划，并说明状态含义与转移。

</Exercise>

<Exercise id="30806" :d="4" :w="3">
...
1. `rows == 0` 或 `columns == 0` 时结果为 0；非空网格指针必须有效。
2. 行列乘积溢出、路径代价加法溢出或参数无效时返回 `false`，输出对象保持不变。
3. 额外空间复杂度必须为 $O(columns)$，时间复杂度为 $O(rows \times columns)$。
4. 说明第一行、第一列和普通格子的边界转移，并用至少一个存在多条等价最优路径的样例测试。

</Exercise>

<Exercise id="30807" :d="4" :w="4">
...
1. 结果必须同时是两个输入的子序列，保持各自在原序列中的相对顺序；结果长度必须最大。输入不是以空字符结尾的字符串，不能调用字符串长度或比较函数。
2. `left_count == 0` 或 `right_count == 0` 时成功返回长度 0；对应指针可以为 `NULL`。非空序列指针和 `out_length` 必须有效；当最大长度大于 0 时，`out_sequence` 也必须有效且 `capacity` 足够。
3. 参数无效、行列乘积溢出或辅助存储分配失败时返回 `false`，不得修改输出数组和 `*out_length`。
4. 使用完整的二维动态规划表，时间复杂度为 $O(nm)$，额外空间复杂度为 $O(nm)$；写出状态含义、相等字节时的转移，以及从表格回溯到输出数组的过程。
5. 当两个候选子问题长度相等时采用固定的决策（例如回溯时优先向上），使相同输入总能得到相同的输出；输出数组不追加空字符。
