...
这一节的完整实现已经放在示例代码目录：[`/教程/示例代码/开发与工程基础/json_parser.c.md`](/教程/示例代码/开发与工程基础/json_parser.c)。同一份实现也已同步到代码目录：`/code/开发与工程基础/20_JSON解析器工程化/json_parser.c`。实现包含以下能力：对象、数组、字符串、数字、`true`、`false`、`null`，并提供统一 `json_free` 深度释放。
