...
**<stddef.h>** 定义了一些跨平台、跨实现的通用类型、宏和常量，几乎所有 C 程序都会直接或间接用到它们。
...
- **定义目的**：用于表示宽字符（wide character），支持更大的字符集（如 Unicode）。
...
## 4. max_align_t —— 最大对齐要求的标量类型（C11 及以后引入）
...
`<stddef.h>` 虽然内容不多，但它是 C 语言的“基石”之一——它提供的类型和宏保证了代码的**可移植性**和**安全性**。在实际编程中：
