...
::: concept ascii-definition title="ASCII 的定义"

ASCII (American Standard Code for Information Interchange) 是一种使用 7 位表示 128 个字符的字符编码标准，把字母、数字、标点符号和控制码映射为数字形式。这种标准化使不同制造商生产的计算机、终端、打印机及其他外围设备能够可靠地交换和处理文本信息。

:::

## 1. 为什么 C 教程要讲 ASCII

::: concept c-charset-guarantees title="C 标准对字符集的保证边界"

C 标准并不要求源字符集或执行字符集必须就是 ASCII。标准要求实现定义源字符集和执行字符集，并规定它们都必须包含基本字符集：大小写拉丁字母、十进制数字、一批图形字符、空格，以及若干控制字符。执行字符集中这些字符的具体编码值由实现定义。标准保证十进制数字字符的编码值连续递增，且字符常量和字符串字面量会在翻译阶段映射到执行字符集。
...
::: generate ascii-vs-c-charset kind=explanation concepts=ascii-definition,c-charset-guarantees

面向刚开始学习 C 的读者，用 ASCII 建立“字符可以映射到整数编码”的直观模型，同时划清可移植边界：`'9' - '0'` 得到 `9`，依据的是标准对十进制数字字符连续递增的保证；ASCII 中 `'0'` 的编码值 `0x30` 只是常见实现上的具体数值。说明为什么可移植代码应依赖标准保证而非 ASCII 码值。
...
::: concept escape-and-null-char title="转义字符与空字符"

写转义字符时，程序表达的是执行字符集中的字符意图，而不是具体编码值：例如 `'\n'` 表示执行字符集中的换行控制字符，通常平台上它对应 `0x0A`，但可移植代码应使用转义字符本身表达意图。标准要求基本执行字符集中存在一个所有位都为 `0` 的空字符（写为 `'\0'`），用于终止字符串。
...
::: generate escape-sequences-intent kind=explanation concepts=escape-and-null-char,c-charset-guarantees

面向刚开始学习 C 的读者，解释为什么可移植代码应使用转义字符本身表达意图，而不是写死编码值；说明 `'\0'` 作为字符串终止符的角色——后续学习字符串库时会反复遇到它，但本节不要展开字符串库细节。
...
| 0011 0110    | 066          | 54           | 0x36           | 6                           | 阿拉伯数字 6 |
