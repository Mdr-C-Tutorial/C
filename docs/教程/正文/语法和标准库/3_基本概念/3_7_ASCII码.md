...
C 标准并不要求源字符集或执行字符集必须就是 ASCII。标准要求实现定义源字符集和执行字符集，并规定它们都必须包含基本字符集：大小写拉丁字母、十进制数字、一批图形字符、空格，以及若干控制字符。执行字符集中这些字符的具体编码值由实现定义。

因此，本节的定位是：用 ASCII 帮你建立“字符可以映射到整数编码”的直观模型，同时提醒你不要把 ASCII 码值误认为所有 C 实现的语言保证。C 标准真正保证的是基本字符集成员存在、数字字符按连续递增方式排列，以及字符常量和字符串字面量会在翻译阶段映射到执行字符集。
...
`'9' - '0'` 得到 `9`，依据的是标准对十进制数字字符连续递增的保证；ASCII 中 `'0'` 的编码值 `0x30` 只是常见实现上的具体数值。
...
很多 C 转义字符来自历史字符设备习惯，例如 `\n` 表示换行，`\t` 表示水平制表，`\0` 表示空字符。需要注意的是，写 `'\n'` 时，程序表达的是“执行字符集中的换行控制字符”。通常平台上它会对应 `0x0A`，但可移植代码应使用转义字符本身表达意图。

```c
char end = '\0';
char newline = '\n';
```
`'\0'` 尤其重要：标准要求基本执行字符集中存在一个所有位都为 `0` 的空字符，用于终止字符串。后续学习字符串库时会反复遇到这一点。
