...
设计一个与位域实际内存布局无关的 32 位交换格式：

- 第 0～7 位：`font_id`；
- 第 8～14 位：`size`；
- 第 15～16 位：`alignment`；
- 第 17、18、19 位：依次表示 `bold`、`italic`、`underline`；
- 第 20～31 位：保留，必须为 0。

要求：

1. `size` 的合法范围是 1～127，`alignment` 只能取三个已定义值。
2. 编码或解码失败时返回 `false`，并且不得修改输出对象。
3. 解码时，保留位非零、字号为 0 或对齐方式非法都视为失败。
4. 只能使用无符号整数上的掩码与移位构造交换格式；不得通过联合体、指针转换或 `memcpy` 复制位域对象表示。
5. 对每个合法样式，先编码再解码应恢复所有字段的值。
6. 说明为什么不能用 `sizeof(TextStyle) == sizeof(uint32_t)` 作为该交换格式可移植的依据。
