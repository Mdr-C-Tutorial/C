...
1. 定义一个 `volatile sig_atomic_t` 对象作为标志位。
2. 在处理函数里只设置该标志位。
...
如果需要使用上述功能，请选择异步信号安全的实现。如在 Linux 中可以使用 `write` 系统调用代替 `printf`，因其被明确列为异步信号安全的。
