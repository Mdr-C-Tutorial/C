...
文本流定位存在限制，且更推荐使用 `fgetpos` / `fsetpos`（见 13.4）。
...
:::
程序同时会在当前工作目录创建或覆盖 `test.txt`，其中保存一行 `Hello, File!`。
## 补充示例：把标准输入复制到文件
