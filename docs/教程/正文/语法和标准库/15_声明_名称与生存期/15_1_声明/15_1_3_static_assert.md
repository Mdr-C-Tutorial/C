...
静态断言最有价值的用法是“贴着被约束对象放置”。例如某个结构要和外部二进制协议对齐，就把 `static_assert(sizeof(...))` 放在该类型定义附近；某个配置宏必须满足范围，就把断言放在相关配置声明附近。这样读者在阅读声明时就能同时看到约束。
