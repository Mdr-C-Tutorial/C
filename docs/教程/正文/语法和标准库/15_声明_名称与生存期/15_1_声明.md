...
`typedef` 是存储类说明符之一。它不改变声明符的解析规则，只是把声明符中的标识符引入为类型别名。

```c
typedef char my_char_t, *my_char_p, (*fp)(void);
```

其中，`my_char_t`、`my_char_p`、`fp` 分别是 `char`、`char*`、`char (*)(void)` 的别名。更完整的用法与风格取舍见 [10.4 typedef](/教程/正文/语法和标准库/10_枚举_结构与联合/10_4_typedef)。

## 4. 本节扩展

本节涉及的若干关键字/特性，在本章后续小节中单独展开：

1. [15.1.1 typeof](/教程/正文/语法和标准库/15_声明_名称与生存期/15_1_声明/15_1_1_typeof)
2. [15.1.2 constexpr](/教程/正文/语法和标准库/15_声明_名称与生存期/15_1_声明/15_1_2_constexpr)
3. [16.3 const volatile](/教程/正文/语法和标准库/16_对象与内存模型/16_3_const与volatile)
4. [16.4 restrict](/教程/正文/语法和标准库/16_对象与内存模型/16_4_restrict)
5. [16.2.3 alignas](/教程/正文/语法和标准库/16_对象与内存模型/16_2_对象表示/16_2_3_alignas)
6. [15.1.3 static_assert](/教程/正文/语法和标准库/15_声明_名称与生存期/15_1_声明/15_1_3_static_assert)
7. [15.3 存储期 链接](/教程/正文/语法和标准库/15_声明_名称与生存期/15_3_存储期与链接)
8. [15.4 定义](/教程/正文/语法和标准库/15_声明_名称与生存期/15_4_定义)

## 5. 习题

<Exercise id="11501" :d="1" :w="2">

用“英语规则（pointer to / array of / function returns）”解释下面每个声明，并写出该标识符的直观含义：
...
<Exercise id="11502" :d="1" :w="2">

写出一个 `typedef`，使得下列声明不再需要括号：`int (*fp)(int, int);`。

</Exercise>

<Exercise id="11503" :d="1" :w="2">

解释为什么 `int f();` 在 C23 之前与 `int f(void);` 不等价；并说明你在项目中应该使用哪一种写法。
