...
::: terminal title="zsh - qsort_strings"
...
::: terminal title="zsh - qsort_fracs"
...
int compare_strings(const void* a, const void* b)
{
    // a 和 b 指向数组元素，数组元素的类型是 const char *
    const char *arg1 = *(const char * const *)a;
    const char *arg2 = *(const char * const *)b;
    return strcmp(arg1, arg2);
}
int main(void)
{
    const char *names[] = {"John", "Paul", "George", "Ringo"};
    size_t size = sizeof names / sizeof names[0];
    qsort(names, size, sizeof names[0], compare_strings);
    const char *key = "George";
    const char **result = bsearch(&key, names, size, sizeof names[0], compare_strings);
    if (result)
        printf("Found %s\n", *result);
    else
        printf("Not found\n");
...
<Exercise id="11802" :d="2" :w="2">

使用 `qsort` 实现下列函数，统计数组中不同整数的数量：
...
函数可以重排数组。`count` 为 0 时返回 0，此时 `items` 可以为 `NULL`；`count` 大于 0 时，`items` 必须指向至少 `count` 个元素。

比较函数不得通过两个整数直接相减来决定返回值，因为减法可能溢出。
