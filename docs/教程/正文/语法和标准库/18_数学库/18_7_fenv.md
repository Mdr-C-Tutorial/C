...
在支持 IEC 60559 浮点异常并遵守 `FENV_ACCESS` 的常见实现中，输出示例为：

::: terminal

FE_INVALID (Invalid operation) flag is set.
Result: nan

:::
NaN 的具体拼写和符号由实现决定，例如也可能显示为 `-nan`。
...
FE_OVERFLOW detected

:::
若实现未定义或未报告 `FE_OVERFLOW`，这段程序不会打印该行。
## 浮点舍入方向
