...
示例见 [9.1 内存模型](/教程/正文/语法和标准库/9_对象模型/9_1_内存模型) 中的 `float` / `uint32_t` 示例。

## 3. 对齐相关

对象表示与对齐密切相关：对齐决定了对象能出现在哪些地址上，也影响结构体内部的填充布局。

1. [9.2.1 对齐](/教程/正文/语法和标准库/9_对象模型/9_2_对象表示/9_2_1_对齐)
2. [9.2.2 alignof](/教程/正文/语法和标准库/9_对象模型/9_2_对象表示/9_2_2_alignof)
...
<Exercise id="10908" :d="1" :w="2">

写程序观察以下类型的 `sizeof` 与 `alignof`（见 9.2.2）：`char`、`int`、`double`、`max_align_t`。
