...
1. 写出 `MAX(i++, j++)` 完全展开后的记号序列。
2. 说明为什么即使每个形参都加了括号，这个宏仍不能保证每个实参只求值一次。
3. 对 `i = 3, j = 5` 和 `i = 5, j = 3` 两组初值，分别推演调用后的结果以及 `i`、`j` 的值。
4. 编写 `static inline int max_int(int left, int right)` 替代该宏，并说明它能解决什么问题、仍不能使哪类调用变得合法。
