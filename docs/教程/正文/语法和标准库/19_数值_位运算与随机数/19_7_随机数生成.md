...
:::
种子来自当前时间，因此每次运行得到的数列可能不同；五个整数都应位于闭区间 `[1, 10]`，最后一个数应位于 `[1.0, 10.0]`。
## 猜数游戏
...
<Exercise id="11908" :d="2" :w="3">

编写一个猜数小游戏：随机生成一个 1 到 100 的整数，玩家需要猜出这个整数。每次猜测后，程序会提示玩家猜测的数是太大还是太小。玩家可以根据提示继续猜测，直到猜中为止。
