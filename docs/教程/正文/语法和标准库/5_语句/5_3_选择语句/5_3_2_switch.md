...
如果在 `switch` 语句内执行时遇到跳转语句 `break;`，则会跳出 switch 语句。我们将在 [5.5.1](/教程/正文/语法和标准库/5_语句/5_5_跳转语句/5_5_1_break_continue.md) 详细介绍 `break;` 语句的其他功能。
...
int a = 1;
const int b = 2;
constexpr int c = 3; /* `constexpr` 用于声明编译期常量，第9章对象模型详细讲解 */
...
在 C23 标准中，有了[属性](/教程/正文/语法和标准库/9_对象模型/9_6_属性.md)的语法，我们可以在 `case` 标签后使用 `[[fallthrough]];` 语句来提醒编译器：这个 fallthrough 是有意为之，而不是漏写了 `break;`，从而避免可能的警告。
...
下面的状态机示例使用了 `enum`，它用于定义一组命名的整数常量。此处照字面理解即可，第6章将系统讲解枚举语法。
...
- 有限种状态（尤其是[枚举](/教程/正文/语法和标准库/6_枚举.md)）
- 每种状态的代码逻辑不同
