...
在 `long` 为常见 32 位或 64 位的实现中，该十进制文本超出表示范围，输出为：
...
:::
如果某个实现提供了足以容纳该值的更宽 `long`，程序会改为打印转换后的数值。
...
<Exercise id="11408" :d="1" :w="2">

写一个小程序：读入一个文件路径，尝试打开并读取第一行；当打开失败时，使用 `perror` 输出原因。

</Exercise>

<Exercise id="11409" :d="1" :w="2">
