...
连续出现的相同字符称为一个游程。长度为 1 的游程直接写出字符；更长的游程写成“十进制长度 + 字符”。例如：
...
1. 未编码文本只包含基本执行字符集中的英文字母和 `<ctype.h>` 所识别的空白，不包含数字。
2. 显式长度必须不小于 2；`0A`、`1A`、`12` 等不是合法编码。
3. 空字符串编码或解码后仍为空字符串。
4. 成功时返回恰好容纳结果的动态数组，由调用方使用 `free` 释放。
5. `input` 为 `NULL`、字符不合法、编码格式错误、大小计算溢出或申请失败时返回 `NULL`。
6. 对每个合法文本 `s`，应满足 `rle_decode(rle_encode(s))` 与 `s` 内容相同。

</Exercise>

<Exercise id="10906" :d="4" :w="3">
...
size_t mdr_strlen(const char *s);
```

返回终止空字符之前的字节数。`s` 指向以空字符结尾的字符串；不得调用 `<string.h>` 中的函数。

</Exercise>

<Exercise id="10908" :d="2" :w="2">

仅使用 C 语言本身提供的操作，实现：

```c
int mdr_strcmp(const char *lhs, const char *rhs);
```

按照 `unsigned char` 的值逐字节比较两个字符串。左侧字符串较小时返回负值，相同时返回 `0`，较大时返回正值；不要求返回固定的 `-1` 或 `1`。不得调用 `<string.h>` 中的函数。

</Exercise>

<Exercise id="10909" :d="1" :w="2">
...
size_t mdr_strnlen(const char *s, size_t maxlen);
```

最多检查 `maxlen` 个字节：若在此范围内遇到空字符，返回它之前的字节数；否则返回 `maxlen`。不得读取给定范围之外的字节，也不得调用 `<string.h>` 中的函数。
