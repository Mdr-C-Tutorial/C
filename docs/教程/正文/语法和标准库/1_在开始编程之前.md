...
不同的是，这里采用的格式是 `[*.*]`（在本网站内用 _斜体_ 标识），其中 `*` 是一个 `0-9` 的数字。第一个 `*` 表示难度等级，第二个 `*` 表示工作量。~~一般地，前面数字大的后面数字不太可能小。~~

难度前面如果加 `M`，说明需要一定的数学知识；如果加 `HM`，说明需要比较高级的数学知识。

难度后面如果加一颗 `*`，表示不做此题几乎对当前章节的学习无影响；如果加两颗 `**`，说明此题涉及当前章节之外（可能是成体系的）知识。

:::

1. [**[10101]**](/教程/题解/语法和标准库/在开始编程之前/10101.md) _[0.1]_ 查找资料，了解高效使用搜索引擎的方法。
2. [**[10102]**](/教程/题解/语法和标准库/在开始编程之前/10102.md) _[0.1]_ 熟悉键盘，知道如何打出下列符号：`+ - _ / | \ ' " : ; , . ? ! ( ) [ ] { } < > & # *`。
3. _[1.3]_ 在电脑上安装 C 语言开发所需的工具链和编辑器。
4. _[0.2*]_ 探索 [Compiler Explorer](https://godbolt.org/)，并了解其功能。
5. _[0.2*]_ 探索 [C++ Refererence](https://zh.cppreference.com/)，并了解其功能。
6. _[3.5*]_ 了解编程语言的发展史和分类。
7. _[5.8**]_ 阅读并学习《计算机程序的构造和解释》（英文书名 Structure and Interpretation of Computer Programs，简称 SICP）。
