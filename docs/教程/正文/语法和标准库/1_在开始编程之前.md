...
7. Visual Studio（简称 VS）是一款及其强大和现代的 IDE，尤其适合 .Net 开发；但由于某些奇怪的设定，以及 MSVC 对 C 的支持有限，VS 不适合用于初学 C 语言。
...
**人工智能** (Artificial Intelligence, AI) 这个词涵盖的领域非常广，但本网站中，它专指**大语言模型** (Large Language Model, LLM)。
...
截至目前（注：2025.7.27），主流的大语言模型提供商有如下几个：
