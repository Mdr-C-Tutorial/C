...
alignof(struct sse_t) = 16

:::
实现可以为该结构体提供更严格的对齐要求，但不得弱于声明请求的 `16`。
## 4. 与内存分配接口的配合
