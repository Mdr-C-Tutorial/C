...
在 `CHAR_BIT` 为 `8`、`unsigned` 为 32 位的常见实现中，小端序和大端序会分别得到：
...
示例见 [16.1 内存模型](/教程/正文/语法和标准库/16_对象与内存模型/16_1_内存模型) 中的 `float` / `uint32_t` 示例。
...
<Exercise id="11607" :d="1" :w="2">

写程序分别观察 `uint16_t`、`uint32_t`、`uint64_t` 的字节序输出，并总结你的平台是大端还是小端。

</Exercise>

<Exercise id="11608" :d="1" :w="2">

写程序观察以下类型的 `sizeof` 与 `alignof`（见 16.2.2）：`char`、`int`、`double`、`max_align_t`。

</Exercise>

<Exercise id="11609" :d="1" :w="2">

设计一个“结构体布局观察”实验：对 `struct { char a; int b; char c; }` 输出：
   1. `sizeof`；
...
<Exercise id="11625" :d="1" :w="2">

使用 `unsigned char` 访问对象表示，实现：

```c
#include <stddef.h>

void *mdr_memset(void *dest, int ch, size_t count);
```

把 `ch` 转换为 `unsigned char` 后写入 `dest` 指向区域的前 `count` 个字节，并返回 `dest`。调用方保证该区域至少包含 `count` 个可写字节；不得调用 `<string.h>` 中的函数。

</Exercise>

<Exercise id="11626" :d="2" :w="2">
...
void *mdr_memcpy(void *restrict dest, const void *restrict src, size_t count);
```

复制恰好 `count` 个字节并返回 `dest`。调用方保证源区域和目标区域均足够大且不重叠；不得调用 `<string.h>` 中的函数。再编写测试，覆盖 `count == 0`、单字节复制和多字节复制。
