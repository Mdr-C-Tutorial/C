...
<Exercise id="10949" :d="1" :w="2">

给你项目里一个准备废弃的函数加上 `[[deprecated("...")]]`，并在说明文字里写出替代 API。

</Exercise>

<Exercise id="10950" :d="1" :w="2">

写一个 `switch`，刻意包含一次 fall-through，并用 `[[fallthrough]];` 标注它。

</Exercise>

<Exercise id="10951" :d="1" :w="2">

找一个“必须检查返回值”的函数（例如解析/打开文件），给它加上 `[[nodiscard]]`，并验证编译器是否给出诊断。
